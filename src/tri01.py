...
for area in [ 'MT' , 'CH' , 'CN' , 'LC' ] :
    ...
    provas = itens['CO_PROVA'].drop_duplicates()
    
    
#    posicao_na_prova_azul = 
#    
#    for item in itens.head().itertuples() :
#        df = itens.loc[(itens['CO_ITEM'] == item.CO_ITEM) & (df['column_name'] <= B)]
#        posicao_na_prova_azul = df['CO_POSICAO']
#    print(row.Index, row.date, row.delay)
#    
#    ACERTAR AQUI - LEMBRAR DAS QUESTES DE NGLES E ESPANHOL
#    
